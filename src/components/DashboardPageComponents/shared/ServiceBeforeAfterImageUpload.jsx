            ...
            <div className="flex items-center gap-x-3">
              <div
                className={`w-10 h-10 flex justify-center items-center border rounded-full font-bold transition-all duration-200 ${
                  imagePairs.length > 0 &&
                  imagePairs.every((pair) => pair.before && pair.after)
        ...
        >
          <div className="ml-4 md:ml-14.5 my-4">
            <h2 className="mb-3 text-[#666]">
              You can upload up to 5 pairs of images.
            </h2>
            <div className="flex gap-5 overflow-x-auto">
              {imagePairs.map((pair, index) => (
                <div
                  key={index}
                  className="mb-6 relative flex gap-3 items-center mt-3 w-80"
                >
                      ...
                      {pair[type] ? (
                        ...
                        >
                          ...
                          <input
                            ...
                            accept="image/*"
                            className="hidden"
                            onChange={(e) => onImageSelect(index, type, e)}
                          />
                        </label>
                      )}
