...
export default function DayView({
  ...
  return (
    <div
      ...
      {/* Resource Headers */}
      <div className="p-2 border-gray-200 grid place-items-center">
        <div
          className={cn(
      ...
      {resourcesWithEvents.length > 0 ? (
        resourcesWithEvents.map((resource, index) => (
          <div
            key={resource.id}
            className={cn(
              "p-4 flex flex-col border-b border-gray-200 items-center gap-2 justify-center bg-primary01/10",
              {
        ...
        <>
          <div
            className={cn("text-center rounded-t-lg min-h-[100px]")}
      ...
      {timeSlots.map((slotTime, rowIndex) => (
        <React.Fragment key={rowIndex}>
          ...
          <div
            className={cn(
              "border-r border-gray-200 bg-gray-50 text-xs font-medium text-gray-500 flex items-center justify-center lg:justify-start",
              {
                "border-b-0": rowIndex === timeSlots.length - 1,
              }
            )}>
            ...
            resourcesWithEvents.map((resource, colIndex) => {
              ...
              return (
                <div
                  key={`${resource.id}-${rowIndex}`}
                  className={cn(
                    "min-h-[100px] border-b border-r border-gray-200 flex flex-col space-y-1",
                    {
                      "last:rounded-br-lg":
                        colIndex === resourcesWithEvents.length - 1,
                    }
