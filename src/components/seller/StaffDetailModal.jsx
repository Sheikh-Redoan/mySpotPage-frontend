...
const ServiceMultiSelect = ({
  ...
  const filteredServices = allAvailableServices.filter(
    (service) =>
          ...
          {filteredServices.length > 0 ? (
            filteredServices.map((service) => (
              <div
                ...
                onMouseDown={() => {
                  // Use onMouseDown to prevent onBlur from closing immediately
...
const StaffDetailModal = ({ staff, onClose, onSave, onRemove }) => {
  ...
  const [formData, setFormData] = useState({
    profileImage:
      staff?.image || "https://placehold.co/80x80/aabbcc/ffffff?text=Staff", // Default placeholder image
  ...
  useEffect(() => {
    if (staff) {
      setFormData({
        profileImage:
          staff.image || "https://placehold.co/80x80/aabbcc/ffffff?text=Staff", // Default placeholder
        firstName: staff.name.split(" ")[0] || "",
        lastName: staff.name.split(" ").slice(1).join(" ") || "",
        phoneNumber: staff.phone || "",
  ...
  const handleShiftToggle = (dayIndex) => {
    setFormData((prev) => {
      ...
      newShifts[dayIndex] = {
        ...
        startTime: newShifts[dayIndex].isOn
          ? ""
          : newShifts[dayIndex].startTime || "09:00",
        endTime: newShifts[dayIndex].isOn
          ? ""
          : newShifts[dayIndex].endTime || "17:00",
        breakOn: newShifts[dayIndex].isOn ? false : newShifts[dayIndex].breakOn,
        breakStartTime: newShifts[dayIndex].isOn
          ? ""
          : newShifts[dayIndex].breakStartTime,
        breakEndTime: newShifts[dayIndex].isOn
          ? ""
          : newShifts[dayIndex].breakEndTime,
      };
  ...
  const handleBreakToggle = (dayIndex) => {
    setFormData((prev) => {
      ...
      newShifts[dayIndex] = {
        ...
        breakStartTime: newShifts[dayIndex].breakOn
          ? ""
          : newShifts[dayIndex].breakStartTime || "12:00",
        breakEndTime: newShifts[dayIndex].breakOn
          ? ""
          : newShifts[dayIndex].breakEndTime || "13:00",
      };
      return { ...prev, workingShifts: newShifts };
    });
  };
  ...
  const allAvailableServices = [
    ...
    "Beach Wave Perm",
    "Keratin Treatment",
    "Keratin Infusion Therapy",
    "Brazilian Blowout",
    "Frizz-Free Smoothing",
    "Management",
    "Client Relations",
    "Strategic Planning",
    "HR",
    ...
    <div className="fixed inset-0 bg-neutral-800/50 flex justify-end items-stretch z-50 overflow-hidden">
      <div className="w-full max-w-full sm:max-w-md md:max-w-lg lg:max-w-2xl xl:max-w-3xl bg-white flex flex-col justify-start items-start overflow-hidden">
        ...
        <form
          ...
          className="self-stretch flex-1 p-5 bg-white flex flex-col gap-5 overflow-auto scrollbar-hidden">
          ...
          {activeTab === "Basic Information" && (
            <div className="self-stretch flex-1 flex flex-col justify-start items-start gap-4">
              ...
              <div className="self-stretch flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                {" "}
                {/* Responsive: flex-col on small, flex-row on medium and up */}
                ...
                <div className="flex justify-start items-center gap-2">
                  {" "}
                  {/* Adjusted flex for just the toggle */}
              ...
              <div className="self-stretch flex flex-col md:flex-row gap-4">
                {/* Responsive: stacks on small, side-by-side on medium and up */}
                <div className="flex-1 flex flex-col justify-start items-start gap-1">
                  <label
                    htmlFor="firstName"
                    className="inline-flex justify-start items-start gap-1">
                  ...
                  {["Male", "Female", "Other"].map((label) => (
                    <label
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        className="size-5 checked:size-3 appearance-none rounded-full border-2 border-gray-300 checked:border-violet-500 checked:bg-violet-500 checked:ring-1 checked:ring-violet-500 checked:ring-offset-4 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:ring-offset-4 transition-all duration-150"
                      />
                      <span className="text-neutral-700 text-sm font-normal font-['Golos_Text'] leading-tight">
                        {label}
                      </span>
                ...
                <div className="self-stretch flex flex-col justify-start items-start gap-6">
                  <div className="self-stretch rounded-xl flex flex-col sm:flex-row justify-start items-start sm:items-center gap-3 sm:gap-6">
                    ...
                    <div className="flex-1 w-full flex justify-start items-center gap-3">
                      ...
                      <div className="w-4 text-center text-neutral-600 text-sm font-normal font-['Golos_Text'] leading-tight">
                        -
                      </div>
                      <div className="flex-1 text-center text-gray-500 text-sm font-normal font-['Golos_Text'] leading-tight">
                        End Shift
                  ...
                  {formData.workingShifts.map((shift, index) => (
                    <div
                      key={shift.day}
                      className="self-stretch rounded-xl flex flex-col sm:flex-row justify-start items-start sm:items-center gap-3 sm:gap-6">
                      {" "}
                      {/* Responsive for each shift row */}
                      ...
                      <div className="flex-1 w-full flex justify-start items-center gap-3">
                        <div
                          ...
                          {shift.isOn ? (
                            ...
                            <span className="text-gray-500 text-sm font-normal font-['Golos_Text'] leading-tight">
                              Unavailable
                            </span>
                          )}
                        </div>
                        <div className="text-center text-neutral-600 text-sm font-normal font-['Golos_Text'] leading-tight">
                          -
                        </div>
                        <div
                          className={`flex-1 self-stretch px-3 py-2 rounded-lg border border-gray-200 flex justify-center items-center gap-2 ${
                  ...
                  {formData.workingShifts.map((shift, index) => (
                    <div
                      ...
                      <label className="w-full sm:w-44 rounded flex justify-start items-center gap-2 cursor-pointer">
                        ...
                        />
                        <span
                          className={`text-sm font-semibold font-['Golos_Text'] leading-tight ${
                            !shift.isOn ? "text-gray-400" : "text-neutral-800"
                          }`}>
                          {shift.day}
                        </span>
                      </label>
        ...
        <div className="self-stretch h-14 px-6 py-4 border-t border-gray-200 flex justify-between items-center">
          ...
          <div className="flex justify-end items-center gap-3">
            {" "}
            {/* Use justify-end to keep save button to the right */}
            <button
              type="submit"
