...
const BookingCart = ({
  ...
  const visibleHours =
    isMobile && !showAll
      ? workingHours.slice(0, maxVisibleItems)
      : workingHours;
  ...
  return (
    <section className="w-full max-w-sm p-5 rounded-xl border border-gray-200 shadow-sm space-y-5 bg-white font-golos mx-auto">
      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className="flex items-center text-sm text-yellow-500 font-medium gap-1">
            ...
            <span className="text-description hover:underline text-sm cursor-pointer">
              <Translator text={`(${reviewCount})`} />
            </span>
          </div>
        </div>
            ...
            {visibleHours.map(({ day, time }) => {
              ...
              return (
                <li
                  ...
                  className={`flex justify-between items-center cursor-pointer rounded-md px-3 py-2 transition ${
                    isSelected
                  ...
                  <span className="flex items-center gap-2">
                    <span
                      className={`w-2 h-2 rounded-full ${
                        isSelected ? "bg-violet-600" : "bg-violet-200"
                      }`}
                    />
                    <Translator text={day} />
                  </span>
                  <span
                    className={
                      isSelected ? "text-primary01 font-semibold" : ""
                    }>
                    <Translator text={time} />
                  </span>
                </li>
              );
            })}
      ...
      <div className="hidden md:block">
        ...
        <button
          className={`w-full bg-gray-900 text-white py-2 rounded-lg transition ${
            selected?.length === 0
