...
const TestimonialCard = ({ name, date, time, avatar, images, description, rating }) => {
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <div className="text-sm text-description">{`${date} • ${time}`}</div>
                </div>
            </div>
            ...
            <div className="flex text-yellow-400">
                {Array(fullStars).fill(0).map((_, i) => (
                    <Star key={`full-${i}`} size={16} fill="currentColor" stroke="currentColor" />
                ))}
                {hasHalfStar && <StarHalf size={16} fill="currentColor" stroke="currentColor" />}
                {Array(emptyStars).fill(0).map((_, i) => (
                    <StarEmpty key={`empty-${i}`} size={16} fill="none" stroke="currentColor" />
                ))}
            </div>
