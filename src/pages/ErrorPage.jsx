  ...
  return (
    <div className="min-h-screen bg-highlight01 flex flex-col items-center justify-center text-center px-4 relative overflow-hidden">
      ...
      <div className="relative">
        <h1 className="text-9xl md:text-[220px] font-bold text-gray-300 leading-none">404</h1>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
          <div className="  -rotate-10">
            <p className="relative inline-block text-lg">
              <span className="relative z-10 block px-3 md:px-7 py-1.5 md:py-3 overflow-hidden font-medium leading-tight text-gray-800 transition-colors duration-300 ease-out border-2 border-border rounded-lg ">
                <span className="absolute inset-0 w-full h-full px-3 md:px-7 py-1.5 md:py-3 rounded-lg bg-primary01"></span>
                <span className="relative text-lg font-bold text-white">Opps!</span>
              </span>
              <span className="absolute bottom-0 right-0 w-full h-[44px] md:h-14 -mb-2 -mr-2 transition-all duration-200 ease-linear bg-gray-900 rounded-lg" data-rounded="rounded-lg"></span>
            </p>
          </div>
        </div>
      </div>
      ...
      <h2 className="text-3xl font-extrabold text-black mt-2">
        Page ot <span className="text-primary01">Found!</span>
      </h2>

      {/* Message */}
      <p className="text-gray-600 mt-4 max-w-xl">
        Looks like this page decided to take a vacation! Don’t worry, even the best websites
        have pages that wander off sometimes. Let’s get you back on track.
      </p>
      ...
      <div className="absolute bottom-4 left-4 rotate-[-6deg]">
        <span className="inline-block bg-red-500 text-white px-3 py-1 rounded-md font-bold shadow-lg border-primary01 border-2">
          Error Code: LOST_IN_SPACE
        </span>
      </div>

      {/* Decorative Icons */}
      <AlertTriangle className="absolute top-1/2 left-5 md:left-10 text-red-300 opacity-40" size={32} />
      <AlertTriangle className="absolute top-[40%] right-5 md:right-10 text-primary01 opacity-40" size={32} />
