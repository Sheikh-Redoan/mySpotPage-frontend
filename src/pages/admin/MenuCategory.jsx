        ...
        {categories.map((cat, catIndex) => (
          <div key={cat.name} className="bg-white rounded-lg">
            <div
              ...
              <span
                className={`${
                  openIndex === catIndex ? "text-primary01 font-semibold" : ""
                }`}>
                {cat.name}
              </span>
              <ChevronUp
                size={18}
                className={`text-gray-500 size-6 transform transition-transform ${
                  openIndex === catIndex ? "rotate-0" : "rotate-180"
                }`}
              />
            </div>
            ...
            {openIndex === catIndex && (
              <div className="px-4">
                <DragDropContext
                  onDragEnd={(result) => handleDragEnd(result, catIndex)}>
                  <Droppable
                    droppableId={`category-${catIndex}`}
                    type="SERVICE">
                    {(provided) => (
                      <table className="w-full table-auto border-separate border-spacing-y-4">
                        ...
                        <tbody
                          ...
                          {cat.services.map((service, serviceIndex) => (
                            <Draggable
                              key={service.id}
                              draggableId={`${cat.name}-${service.id}`}
                              index={serviceIndex}>
                              {(provided) => (
                                <tr
                                  ref={provided.innerRef}
