...
import { useLocation, useNavigate, useSearchParams } from "react-router"; // Corrected import for react-router v6
...
const AddNewServicePage = () => {
    ...
    const handleWorkRemoveImage = (index) => {
        ...
        setWorkCroppedImages(workCroppedImages.filter((_, i) => i !== index));
    };


    // --- NEW ---: Handlers for "Before & After" (Crop-First Workflow)
    ...
    const handleBeforeAfterFileSelect = (e) => {
        const file = e.target.files[0];
        if (file) {
            const imageURL = URL.createObjectURL(file);

            // Store original source for re-cropping
            const { index, position } = croppingTarget;
            const updatedOriginals = [...originalImagePairs];
            // Ensure the pair exists before assigning
            while(updatedOriginals.length <= index) {
                updatedOriginals.push({ before: null, after: null });
            }
            updatedOriginals[index][position] = imageURL;
    ...
    const onRemoveImage = (index, type) => {
        ...
        const updatedOriginals = [...originalImagePairs];
        updatedOriginals[index][type] = null;
        setOriginalImagePairs(updatedOriginals);
    };
    
    // (Other functions like handleCollapseChange, priceCheckboxChange remain the same)
    ...
    return (
        <div className="w-full p-4 md:p-5">
            {/* --- NEW ---: Hidden file input for Before & After */}
            <input
                type="file"
                className="hidden"
                ref={beforeAfterFileInputRef}
                accept="image/*"
                onChange={handleBeforeAfterFileSelect}
            />
            ...
            <div className="flex justify-between items-center">
                ...
                <Button color="default" variant="solid" disabled>Publish</Button>
            </div>
            ...
            <div className="mb-20 md:mb-0">
                <Space direction="vertical" className="w-full my-4 space-y-6">
                    <ServiceBasicDetails
                        ...
                        setIsModalOpen={() => {
                            setCroppingTarget({ type: "basicDetails" });
                            setImageToCrop(thumbnailImage); // Set image for re-cropping
                            setIsModalOpen(true);
                        }}
                    />
            ...
            {/* --- MODIFIED ---: Single, Unified Modal --- */}
            <ImageCropModal
                isOpen={isModalOpen}
                onClose={() => setIsModalOpen(false)}
                image={imageToCrop} // The modal always gets its image from `imageToCrop`
            ...
            <div className="flex justify-between items-center md:hidden fixed bottom-0 left-0 right-0 bg-white pb-10 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] p-5 z-50">
                <button type="button" className=" md:hidden text-white font-semibold bg-gray-900 py-2.5 w-full rounded-lg cursor-pointer hover:scale-95 transform transition-all duration-300 ease-in-out">
                    Publish
                </button>
            </div>
        </div>
